<template>
  <div>
    <div class="tabnavBox">
      <transition-group name="list" tag="ul">
        <li v-for="(item, index) in $store.getters.tabnavBox" @contextmenu.prevent="openMenu(item,$event,index)"
          ...
          <router-link :to="item.path">方法</router-link>
    ...
    <ul v-show="this.rightMenuShow" :style="{left:this.left+'px',top:this.top+'px'}" class="menuBox">
      <li @click="removeTab($store.getters.rightNav)"><i class="fa fa-remove"></i>谷歌</li>
      <li @click="removeOtherTab($store.getters.rightNav)">搜索</li>
      <li @click="removeAllTab">方法</li>
    </ul>
  </div>
</template>
